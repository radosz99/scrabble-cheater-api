from cheater_app.logic.structures import Orientation
from cheater_app.logic.pattern_finder import check_if_cell_has_got_no_vertical_neighbors, check_if_cell_has_got_no_horizontal_neighbors
from cheater_app.logic.exceptions import IncorrectMoveException
    ...
    for letter in [letter for letter in move.get_letters_list() if letter.is_user_letter()]:
        if move.get_orientation() == Orientation.VERTICAL:
            if not check_if_cell_has_got_no_horizontal_neighbors(board, letter.get_x(), letter.get_y()):
                raise IncorrectMoveException(f"ERROR - {move}, {move.get_pattern()}")
